...
<template>
    <div class="relative">
        <div class="flex items-center w-24 justify-between border py-1 px-2 rounded-lg cursor-pointer select-none"
        ...
        <Transition>
            <div class="bg-white select-none border rounded-lg mt-2 absolute z-50 py-1" v-if="isOpen">
                <div v-for="item in props.options" :key="item.id" @click.stop="clickOption(item)"
                    class="py-1.5 cursor-pointer hover:bg-slate-200 duration-150 px-2 rounded-md flex items-center justify-start gap-2 w-24">
